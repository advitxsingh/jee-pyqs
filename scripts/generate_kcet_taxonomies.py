"""
Generator script for authentic KCET Syllabus Taxonomy.
Constructs dedicated, high-precision taxonomies for all 95 KCET chapters
based on actual KCET PYQs, Karnataka PUC syllabus, and exam characteristics.
"""

import os
import json

KCET_TAXONOMY_CONTENT = '''"""
Dedicated KCET Concept Taxonomy.
Crafted specifically for the Karnataka CET exam pattern:
- 60 questions in 80 minutes (speed, direct formula recall, NCERT text lines)
- No negative marking: fast substitution, dimensional elimination, ratio shortcuts
- Exact chapter splits as defined in Karnataka Pre-University (PUC I & II) syllabus
"""
...
KCET_TAXONOMY: Dict[str, List[Dict[str, Any]]] = {
    ...
    "kcet-linear-programming": [
        {
            ...
            "common_traps": "1. If the feasible region is unbounded, the minimum or maximum value of $Z$ may not exist unless the open half-plane has no common points with the feasible region. 2. Remember that in an LPP, the objective function is strictly linear.",
            "tips_and_tricks": "In KCET, always sketch the constraint boundary lines quickly using intercept form $\\frac{x}{a} + \\frac{y}{b} = 1$ to find the feasible polygon in under 30 seconds."
        },
        {
            ...
            "summary": "Corner Point Method for optimization: If an optimal value exists, it occurs at a vertex (corner point). If the maximum/minimum occurs at two distinct corner points, it attains the same optimal value at every point along the line segment joining them.",
            "standard_formulas": r"\text{If } Z(A) = Z(B) = Z_{\max}, \text{ then } Z \text{ is maximized at all points on segment } AB",
            "common_traps": "Assuming optimal solution is only at integer points—if not specified as an integer programming problem, fractional coordinates of corner points are valid.",
            "tips_and_tricks": "KCET Shortcut: When $Z = px + qy$ attains the same maximum at $(x_1, y_1)$ and $(x_2, y_2)$, simply equate $p x_1 + q y_1 = p x_2 + q y_2$ to find the ratio $p/q$ directly!"
        }
    ],
    ...
    "kcet-elasticity": [
        {
            ...
            "tips_and_tricks": "For two wires under equal tension: $\\frac{\\Delta L_1}{\\Delta L_2} = \\left(\\frac{L_1}{L_2}\\right) \\left(\\frac{d_2}{d_1}\\right)^2 \\left(\\frac{Y_2}{Y_1}\\right)$. This ratio relation solves 60% of KCET elasticity questions."
        },
        {
    ...
    "kcet-fluid-mechanics": [
        ...
        },
        {
            ...
            "tips_and_tricks": "In horizontal pipes, $\\Delta P = \\frac{1}{2}\\rho (v_2^2 - v_1^2)$. If radius is halved, area is $1/4$, velocity is $4\\times$."
        },
        {
            ...
            "common_traps": "1. Terminal velocity depends on $r^2$, NOT $r$! If radius is doubled, $v_t$ quadruples. 2. A bubble inside a liquid has only 1 surface (excess pressure $2T/R$), while a soap bubble in air has 2 surfaces ($4T/R$).",
    ...
    "kcet-circular-motion": [
        {
            ...
            "summary": "Circular motion relations: $v = \\omega r$, centripetal acceleration $a_c = \\frac{v^2}{r} = \\omega^2 r$. In uniform circular motion (UCM), speed and kinetic energy are constant, but velocity and acceleration vectors continuously change direction.",
    ...
    "kcet-differentiation": [
        {
            ...
            "common_traps": "Check the given interval of $x$ (e.g. $1/2 < x < 1$) before writing the simplified inverse trigonometric form to ensure the angle stays in the principal domain.",
            "tips_and_tricks": "Derivative of $u(x)$ with respect to $v(x)$ is simply $\\frac{du/dx}{dv/dx}$. Simplify both $u$ and $v$ using trig identities before taking derivatives."
        },
        {
            ...
            "common_traps": "For parametric second derivatives, students often forget the final chain rule step $\\times \\frac{dt}{dx}$!",
            "tips_and_tricks": "For homogeneous implicit equations of type $x^p y^q = (x+y)^{p+q}$, $\\frac{dy}{dx}$ is always identically equal to $\\frac{y}{x}$, and $\\frac{d^2y}{dx^2} = 0$!"
        }
    ],
    ...
    "kcet-indefinite-integration": [
        {
            ...
            "common_traps": "Forgetting the factor $\\frac{1}{a}$ in $\\frac{1}{a}\\tan^{-1}\\frac{x}{a}$, or putting $1/a$ in $\\sin^{-1}\\frac{x}{a}$ (which does NOT have $1/a$!).",
            "tips_and_tricks": "For $\\int e^x [f(x) + f'(x)] dx$, the answer is immediately $e^x f(x) + C$. This appears in almost every second KCET paper."
        },
        {
            ...
            "summary": "Integration by parts $\\int u v dx = u \\int v dx - \\int (u' \\int v dx) dx$ guided by the ILATE priority rule (Inverse, Logarithmic, Algebraic, Trigonometric, Exponential). Resolution into partial fractions for proper rational functions.",
            "standard_formulas": r"\int u v dx = u \int v dx - \int \left(\frac{du}{dx} \int v dx\right) dx, \quad \int e^x[f(x) + f'(x)]dx = e^x f(x) + C",
            "common_traps": "Applying partial fractions to improper rational functions without first dividing numerator by denominator.",
            "tips_and_tricks": "In KCET, differentiate the four given options if finding the integral takes more than 1 minute. Differentiating options is often 2x faster!"
        }
    ],
    ...
    "kcet-definite-integration": [
        {
            ...
            "tips_and_tricks": "KCET Classic: Any integral of the form $\\int_a^b \\frac{f(x)}{f(x) + f(a+b-x)} dx$ is directly equal to $\\frac{b-a}{2}$! Instant 5-second answer."
        }
    ],
    ...
    "kcet-aldehyde-and-ketone": [
        {
            ...
            "tips_and_tricks": "Reactivity order toward nucleophilic addition: $\\text{HCHO} > \\text{RCHO} > \\text{RCOR}$ (steric hindrance and $+I$ effect reduce electrophilicity of carbonyl carbon)."
        },
        {
            ...
            "summary": "Aldol condensation requires $\\alpha$-hydrogen (forms $\\beta$-hydroxyaldehyde/ketone $\\to \\alpha,\\beta$-unsaturated carbonyl). Cannizzaro occurs for aldehydes lacking $\\alpha$-H (disproportionation into alcohol + carboxylate). Iodoform test: positive for compounds containing $\\text{CH}_3\\text{CO-}$ or $\\text{CH}_3\\text{CH(OH)-}$ group. Tollens' & Fehling's tests oxidize aldehydes, not ketones.",
            ...
            "common_traps": "Acetophenone ($\text{C}_6\text{H}_5\text{COCH}_3$) gives positive iodoform test (has methyl ketone), while benzophenone ($\text{C}_6\text{H}_5\text{COC}_6\text{H}_5$) does not.",
            "tips_and_tricks": "Clemmensen uses acidic conditions (Zn-Hg / conc. HCl) $\\implies$ suitable for acid-stable groups. Wolff-Kishner uses basic conditions ($\\text{NH}_2\\text{NH}_2$ / KOH / glycol) $\\implies$ suitable for base-stable groups."
        }
    ],
    ...
    "kcet-carboxylic-acids-and-its-derivatives": [
        {
            ...
            "summary": "Carboxylic acids are much more acidic than phenols because the negative charge in carboxylate ion is delocalized over two highly electronegative oxygen atoms. Electron withdrawing groups ($-I, -M$) like $-\\text{NO}_2, -\\text{Cl}, -\\text{CN}$ increase acidity; electron donating groups ($+I, +M$) decrease acidity.",
            "standard_formulas": r"\text{Acidity: } \text{HCOOH} > \text{CH}_3\text{COOH} > \text{CH}_3\text{CH}_2\text{COOH}; \quad \text{Cl}_3\text{CCOOH} > \text{Cl}_2\text{CHCOOH} > \text{ClCH}_2\text{COOH}",
            "common_traps": "Formic acid (HCOOH) is more acidic than acetic acid ($\text{CH}_3\text{COOH}$) because the methyl group exerts an acid-weakening $+I$ inductive effect.",
            "tips_and_tricks": "Carboxylic acids effervesce with $\\text{NaHCO}_3$ (liberating $\\text{CO}_2$ gas), whereas phenols (except picric acid) do not react with sodium bicarbonate."
        },
        {
    ...
    "kcet-general-organic-chemistry": [
        ...
        },
        {
            ...
            "tips_and_tricks": "In gas phase, amine basicity follows strictly $+I$: $3^\circ > 2^\circ > 1^\circ > \\text{NH}_3$. In aqueous medium for ethyl: $2^\circ > 3^\circ > 1^\circ > \\text{NH}_3$ (231); for methyl: $2^\circ > 1^\circ > 3^\circ > \\text{NH}_3$ (213)."
        }
    ],
    ...
    "kcet-isomerism": [
        {
            ...
            "tips_and_tricks": "Boiling points of chain isomers decrease with increasing branching because surface area decreases, weakening van der Waals forces: $n\\text{-pentane} > \\text{isopentane} > \\text{neopentane}$."
        },
        {
            ...
            "tips_and_tricks": "Meso compounds contain chiral carbons but possess an internal plane of symmetry, making them optically inactive by internal compensation."
        }
    ],
    ...
    "kcet-iupac-nomenclature": [
        {
            ...
            "standard_formulas": r"\text{IUPAC Format: } \text{Prefix (substituents)} + \text{Root Word (chain length)} + \text{Primary Suffix (saturation)} + \text{Secondary Suffix (principal group)}",
            ...
            "tips_and_tricks": "In KCET, look at the end of the 4 options first (the principal suffix like -oic acid vs -oate vs -al). This usually eliminates 2 options instantly."
        }
    ],
    ...
    "kcet-metallurgy": [
        ...
        },
        {
            ...
            "common_traps": "In Hall-Héroult process, the carbon anodes are consumed by reacting with liberated oxygen to form $\\text{CO}$ and $\\text{CO}_2$, needing periodic replacement.",
            "tips_and_tricks": "Zone refining is based on the principle that impurities are more soluble in the molten state (melt) than in the solid state of the metal."
        }
    ],
    ...
    "kcet-polymers": [
        {
            ...
            "common_traps": "Nylon 6 is made from caprolactam alone (a single monomer with 6 carbons), while Nylon 6,6 is made from two different 6-carbon monomers.",
            "tips_and_tricks": "Thermoplastics soften on heating (polythene, PVC, polystyrene); thermosetting plastics undergo irreversible cross-linking and do not soften (Bakelite, melamine)."
        },
        {
            ...
            "summary": "Biodegradable polymers: PHBV (poly $\\beta$-hydroxybutyrate-co-$\\beta$-hydroxyvalerate) from 3-hydroxybutanoic acid + 3-hydroxypentanoic acid; Nylon 2-nylon 6 from glycine + amino caproic acid. Natural rubber is cis-1,4-polyisoprene. Vulcanization involves heating raw rubber with 5% sulphur to introduce cross-links and enhance tensile strength.",
            "standard_formulas": r"\text{Natural Rubber: } [\text{-CH}_2\text{-C(CH}_3)\text{=CH-CH}_2\text{-}]_n \quad (\text{cis-isomer})",
            "common_traps": "Natural rubber is the cis-isomer of 1,4-polyisoprene; the trans-isomer is Gutta-Percha, which is hard and non-elastic.",
    ...
    "kcet-chemistry-in-everyday-life": [
        {
            ...
            "summary": "Analgesics relieve pain: non-narcotics (aspirin, paracetamol) inhibit prostaglandin synthesis; narcotics (morphine, codeine) are habit-forming pain-relievers. Antiseptics (applied to living tissues): Dettol (chloroxylenol + terpineol), bithionol (added to soaps), tincture of iodine (2-3% iodine in alcohol-water). Disinfectants (applied to inanimate objects): 1% phenol solution (0.2% phenol is antiseptic).",
            ...
            "tips_and_tricks": "Bactericidal antibiotics kill bacteria (penicillin, ofloxacin, aminoglycosides); bacteriostatic antibiotics inhibit bacterial growth (erythromycin, tetracycline, chloramphenicol)."
        },
        {
            ...
            "summary": "Artificial sweeteners: Aspartame (unstable at cooking temperatures, used in cold foods/soft drinks), Saccharin (550x sweeter than sugar), Sucralose (trichloro derivative of sucrose, stable at cooking temperatures), Alitame (high potency, 2000x sweeter). Detergents: Anionic (sodium lauryl sulphate, in toothpastes), Cationic (cetyltrimethylammonium bromide, has germicidal properties in hair conditioners), Non-ionic (polyethylene glycol stearate, in liquid dishwashers).",
...
with open("app/analyzer/taxonomies/kcet_taxonomy.py", "w", encoding="utf-8") as f:
    f.write(KCET_TAXONOMY_CONTENT)

print("Generated app/analyzer/taxonomies/kcet_taxonomy.py successfully!")
